...
Page({
  ...
  keywordSearch: function (e) {
    ...
    var that = this;
    var urlStr = app.globalData.url + 'messenger/selectUnsubmit';
  ...
  submit: function (e) {
    ...
    // console.log('序列是:' + app.9globalData.myInfo.unsumbit_problem[1]);
    if (this.data.employees[index_text].isAssignmentTeacher == '1') {
      // console.log("进入中")
      this.setData({
        showModal: true,
        is_show_wait: '请耐心等待...',
        img_process2: 'https://lg-52bo0bvm-1253678560.cos.ap-shanghai.myqcloud.com/gou2.jpg',
        assignment_time: util.conversionTime(this.data.employees[index_text].assignmentTime),
        process_time: util.conversionTime(util.conversionTime(this.data.employees[index_text].processTime)),
        confirm_time: util.conversionTime(util.conversionTime(this.data.employees[index_text].confirmTime)),
      })
    }

    if (this.data.employees[index_text].isProcess == '1') {
      this.setData({
        showModal: true,
        is_show_wait: '',
    ...
    if (this.data.employees[index_text].isConfirm == '1') {
      this.setData({
        showModal: true,
        img_process5: 'https://lg-52bo0bvm-1253678560.cos.ap-shanghai.myqcloud.com/gou2.jpg'
      })
    }
    if (this.data.employees[index_text].isAgree == '1') {
      this.setData({
        showModal: true,
    ...
    this.setData({
      ...
      sumbit_time: util.conversionTime(this.data.employees[index_text].sumbitTime)
    })
  },
  go: function () {
    this.setData({
  ...
  onLoad: function () {
    this.setData({
      employees: app.globalData.myInfo.unsumbit_problem,
    })
  }
})
