...
Page({
  ...
  fetchSearchList: function () {
    ...
    let problemType = that.data.problemType,//输入框字符串作为参数
      ...
      teacherAccount = app.globalData.myInfo.process_teacher_imformation.processTeacherAccount; //教师的账号
    console.log('页数' + problemType + searchKeyword)
    //访问网络
    util.getProblem(problemType, searchPageNum, callbackcount, searchKeyword, 'unConfirm', teacherAccount, function (data) {
      console.log(data)
      //判断是否有数据，有则取数据
  ...
  onLoad: function () {
    var that = this
    this.data.teacherAccount = app.globalData.myInfo.process_teacher_imformation.processTeacherAccount;
    this.data.problemType = app.globalData.myInfo.process_teacher_imformation.processTeacherType;
    that.fetchSearchList();
    console.log('问题类型是:' + this.data.problemType);
    that.setData({
      searchLoading: false,
