...
Page({
  ...
  finashAction: function () {
    var processTeacherAccount = app.globalData.myInfo.process_teacher_imformation.processTeacherAccount;
    var email = this.data.email;
    ...
    var urlStr = app.globalData.url + 'processTeacher/bindEmail';
    wx.request({
      method: "POST",
      url: urlStr, //仅为示例，并非真实的接口地址
      data: {
  ...
  onLoad: function (options) {
    var that = this;
    if (app.globalData.myInfo.process_teacher_imformation.email == '' || app.globalData.myInfo.process_teacher_imformation.email == null) {
      this.setData({
        email: '暂无'
      })
    } else {
      this.setData({
        email: app.globalData.myInfo.process_teacher_imformation.email
      })
    }
    this.setData({
      numShow: '',
