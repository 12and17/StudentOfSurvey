...
Page({
  ...
  fetchSearchList: function () {
    ...
    //访问网络
    util.getSearchMusic(problemType, searchPageNum, callbackcount, searchKeyword, 'unconfirm', function (data) {
      console.log(data)
      //判断是否有数据，有则取数据
  ...
  submit: function (e) {
    ...
    var urlStr = app.globalData.url + 'charge/confirmProblem';
    wx.request({
      dataType: "json",
      method: "POST",
      ...
      success: function (res) {
        if (res.data.success == true) {
          ...
          app.globalData.myInfo.count_num.noConfiremTeacherNum = app.globalData.myInfo.count_num.noConfiremTeacherNum - 1;
          if (that.data.problemType === 'richangjiaoxue') {
            app.globalData.myInfo.count_num.noConfiremOfTeachNum = app.globalData.myInfo.count_num.noConfiremOfTeachNum - 1
          } else if (that.data.problemType === 'shebei') {
            app.globalData.myInfo.count_num.noConfiremOfEquipmentNum = app.globalData.myInfo.count_num.noConfiremOfEquipmentNum - 1
          } else if (that.data.problemType === 'xuqiu') {
            app.globalData.myInfo.count_num.noConfiremOfDemandNum = app.globalData.myInfo.count_num.noConfiremOfDemandNum - 1
          }
          that.fetchSearchList();
        } else {
          that.setData({
  ...
  determine2: function () {
    ...
    var urlStr = app.globalData.url + 'charge/toTeacher';
    wx.request({
      dataType: "json",
      method: "POST",
      url: urlStr, //仅为示例，并非真实的接口地址
      data: {
        problemId: this.data.problemId,
        toTeacherReason: this.data.rejectReason
      },
      header: {
