...
Page({
  ...
  // 页面加载
  onLoad: function () {
    if (app.globalData.myInfo.count_num.noConfiremOfTeachNum > 0){
      this.setData({
        teach_num: app.globalData.myInfo.count_num.noConfiremOfTeachNum,
        is_show1: true
      })
    }
    if (app.globalData.myInfo.count_num.noConfiremOfEquipmentNum > 0){
      this.setData({
        equipment_num: app.globalData.myInfo.count_num.noConfiremOfEquipmentNum,
        is_show2: true
      })
    }
    if (app.globalData.myInfo.count_num.noConfiremOfDemandNum){
      this.setData({
  ...
  getAction: function (e) {
    wx.redirectTo({
      url: '../showUnconfirmProblem/showUnconfirmProblem?type=' + e.target.id
    })
  }
})
