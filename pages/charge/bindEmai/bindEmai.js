...
Page({
  ...
  finashAction: function () {
    var chargeAccount = app.globalData.myInfo.charge_imformation.chargeAccount;
    var email = this.data.email;
    ...
    var urlStr = 'http://localhost:8080/charge/modifyEmail';
    wx.request({
      method: "POST",
      url: urlStr, //仅为示例，并非真实的接口地址
      data: {
  ...
  onLoad: function (options) {
    var that = this;
    if (app.globalData.myInfo.charge_imformation.email == '' || app.globalData.myInfo.charge_imformation.email == null) {
      this.setData({
        email: '暂无'
      })
    } else {
      this.setData({
        email: app.globalData.myInfo.charge_imformation.email
      })
    }
    this.setData({
      numShow: '',
